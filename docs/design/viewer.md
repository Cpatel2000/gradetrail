...
1. **The viewer joins results with the dataset; results files are not changed.**
   `results.jsonl` stores outputs (state, score, response_text, tokens, detail) but not inputs (no question text, no expected answer). The viewer loads the run's `manifest.json`, resolves the spec and dataset it names, and joins on `sample_id`. If the dataset file's current sha256 does not match the manifest's `dataset_sha256`, the viewer still renders but shows a visible "dataset changed since this run" warning on the run. Making results self-describing (embedding rendered prompt / sample fields in SampleResult) is a separate gradetrail 0.2+ roadmap item, not part of the viewer.
